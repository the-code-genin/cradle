...
class AuthController {
    public static function signup(Request $request, Response $response, array $args): Response
    {
        ...
        if (!$v->validate()) {
            $response->getBody()->write(new InvalidFormDataError(array_shift($v->errors())[0]));
            return $response->withStatus(400)->withHeader('Content-Type', 'application/json');
        } else if (User::where('email', $body->email)->count() != 0) {
            $response->getBody()->write(new ConflictError('This email is taken.'));
        ...
        try {
            ...
            $response->getBody()->write(new ServerError($e->getMessage()));
            return $response->withStatus(500)->withHeader('Content-Type', 'application/json');
        }


        // Return the response
        $response->getBody()->write(new ApiResponse([
            'data' => $user->fresh(),
    ...
    public static function login(Request $request, Response $response, array $args): Response
    {
        ...
        if ($user == null) {
            $response->getBody()->write(new NotFoundError('No user was found with the email and password combination.'));
